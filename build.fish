...
docker build -t $REPOSITORY/$USER/$IMAGE_NAME:$BACKEND_TAG -f $BACKEND_DIR/Dockerfile $BACKEND_DIR
docker push $REPOSITORY/$USER/$IMAGE_NAME:$BACKEND_TAG
docker push $REPOSITORY/$USER/$IMAGE_NAME:latest-backend

# docker build -t $REPOSITORY/$USER/$IMAGE_NAME:$FRONTEND_TAG -f $FRONTEND_DIR/Dockerfile $FRONTEND_DIR

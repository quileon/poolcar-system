"""
QoS (Quality of Service) Analysis Tool
======================================

This script analyzes network QoS metrics including:
- Connection retries
- Latency (min, max, average, median)
- Frozen datetime detection and handling
- Packet loss events
- Graph visualization
...
Configuration:
    Edit the configuration section below to adjust test parameters.
"""
...
def analyze_qos(start_test_id, end_test_id, csv_file):
    ...
    - Frozen datetimes: detect and exclude (NaN, 1980 dates, duplicates)
    - Latency: datetime_receive - datetime_send (excluding frozen records)
    - Packet Loss: when connection_sequence_id iteration is skipped

    Args:
        start_test_id (int): Starting test ID
        end_test_id (int): Ending test ID
        csv_file (str): Path to CSV file

    Returns:
        dict: Dictionary containing analysis results and dataframe
    """
    ...
    if not valid_latencies.empty:
        ...
        std_latency = valid_latencies.std()
    else:
        min_latency = max_latency = avg_latency = median_latency = std_latency = 0
    ...
    print(f"  Std Dev: {std_latency:.4f}s")
    ...
    loss_rate = (total_loss_count / len(df)) * 100 if len(df) > 0 else 0
...
def plot_sequence_id_with_loss(
    ...
    plt.tight_layout()
    plt.savefig(output_file, dpi=GRAPH_DPI, bbox_inches="tight")
...
def save_summary_csv(results, output_file="qos_summary.csv"):
    """Save analysis summary to CSV file."""
    summary_data = {
        "Metric": [
            "Total Records",
            "Frozen Datetimes",
            "Valid Records for Analysis",
            "Connection Retries",
            "Retry Events",
            "Min Latency (s)",
            "Max Latency (s)",
        ...
        "Value": [
            len(results["df"]),
            results["frozen_count"],
            len(results["valid_latencies"]),
            results["total_retries"],
            results["retry_events"],
            f"{results['min_latency']:.4f}",
            f"{results['max_latency']:.4f}",
            f"{results['avg_latency']:.4f}",
            f"{results['median_latency']:.4f}",
            f"{results['std_latency']:.4f}",
            len(results["packet_loss_events"]),
            results["total_loss_count"],
            f"{results['loss_rate']:.2f}",
        ],
    }
...
def main():
    """Run QoS analysis and generate graphs."""
    ...
    plot_sequence_id_with_loss(
        df,
        results["packet_loss_events"],
        test_ids,
        output_file=f"{OUTPUT_DIR}/qos_sequence_loss.png",
    )
    plot_latency_over_time(
        df,
        results["valid_latencies"],
        test_ids,
        output_file=f"{OUTPUT_DIR}/qos_latency.png",
    )

    # Save summary CSV
    save_summary_csv(results, output_file=f"{OUTPUT_DIR}/qos_summary.csv")
